...
export default function OutgoingEditor() {
  ...
  const loadMessages = async () => {
    try {
      const res = await axios.get(`${baseUrl}/api/messages/outgoing`);
  ...
  const addMessage = async () => {
    ...
    try {
      const res = await axios.post(`${baseUrl}/api/messages/outgoing`, { message });
      setMessages(res.data.messages);
      setMessage('');
  ...
  const setAsDefault = async (msg) => {
    try {
      await axios.put(`${baseUrl}/api/messages/outgoing/default`, { message: msg.message });
  ...
  const deleteMessage = async (id) => {
    try {
      const res = await axios.delete(`${baseUrl}/api/messages/outgoing/${id}`);
      setMessages(res.data.messages);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>Outgoing Call Messages</Text>
      <TextInput
        style={styles.input}
        multiline
