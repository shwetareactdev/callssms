...
export default function IncomingEditor() {
  ...
  const loadMessages = async () => {
    try {
      const res = await axios.get(`${baseUrl}/api/messages/incoming`);
  ...
  const addMessage = async () => {
    ...
    try {
      const res = await axios.post(`${baseUrl}/api/messages/incoming`, { message });
      setMessages(res.data.messages);
      setMessage('');
  ...
  const setAsDefault = async (msg) => {
    try {
      await axios.put(`${baseUrl}/api/messages/incoming/default`, { message: msg.message });
  ...
  const deleteMessage = async (id) => {
    try {
      const res = await axios.delete(`${baseUrl}/api/messages/incoming/${id}`);
      setMessages(res.data.messages);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>Incoming Call Messages</Text>
      <TextInput
        style={styles.input}
        multiline
